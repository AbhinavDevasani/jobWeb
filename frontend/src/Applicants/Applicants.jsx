...
function JobList() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/users/getApplicants")
  ...
  return (
    ...
    <div>
    <div className="grid grid-cols-4 gap-4 text-gray-600 font-medium p-6">
          <div>Applicant</div>
          <div>Contact</div>
          <div>Company</div>
          <div>Location</div>
  ...
  {applicants.map((item, id) => (
    <div key={id} className="p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mt-4 border-t border-gray-200">
        ...
        <div className="grid grid-cols-4 gap-4 py-3 border-t border-gray-100 items-center">
          <h1 className="font-semibold text-gray-800">{item.name}</h1>
          <div>
            <div className="flex items-center space-x-2">
              <span>📧</span>
              <a href={`mailto:${item.email}`} className="text-blue-600">{item.email}</a>
            </div>
            <div className="flex items-center space-x-2 mt-1">
              <span>📞</span>
              <a href={`tel:${item.number}`} className="text-gray-700">{item.number}</a>
            </div>
          </div>
          <div className="text-gray-700">{item.jobId.company}</div>
          <div className="text-gray-700">{item.jobId.location}</div>
        </div>
      </div>
    </div>
  ))}
